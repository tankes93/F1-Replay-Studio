    ...
    def draw(self, window):
        ...
        for i, (code, color, pos, progress_m) in enumerate(self.entries):
            ...
            arcade.Text(text, left_x, top_y, text_color, 16, anchor_x="left", anchor_y="top").draw()
             ...
             # Tyre Icons
            tyre_value = pos.get("tyre", -1)
            # Convert tyre int to filename format (e.g., 0 -> "0.0")
            tyre_key = f"{float(tyre_value)}" if tyre_value >= 0 else None
            tyre_texture = self._tyre_textures.get(tyre_key) if tyre_key else None
            if tyre_texture:
